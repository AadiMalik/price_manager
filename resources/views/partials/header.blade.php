...
<nav class="navbar">
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="{{route('home')}}">Price
                Manager   @if(auth()->user()->userType) {{auth()->user()->userType->name}}
                 @else
...
@if(!in_array(auth()->user()->user_type,[1,2]))
    @php
        $startDate = strtotime(now());
                $endDate = strtotime(auth()->user()->expiry_date);

    $datediff = '';
    $remainingDay = 0;
                if($startDate <= $endDate){
                    $datediff = $endDate - $startDate;
                    $remainingDay = floor($datediff / (60 * 60 * 24));
                }
    @endphp
    <div class="row">
        <div class="col-md-3"></div>
        <div class="col-md-9 text-center" style="padding-top: 70px; font-size:30px; font-weight:bold;"><h2>Remaining days: {{$remainingDay}}
            </h2></div>
    </div>
@endif
<!-- #Top Bar -->